param(
  ...
  [string]$LogPath = "Temp/toontown-smoke.log"
)
...
$args = @(
  ...
  "-executeMethod", "Toontown.Editor.Validation.ToontownSampleSmokeTestRunner.Run",
  "-logFile", $logFullPath
)

Write-Host "=== Toontown Smoke (Batch) ===" -ForegroundColor Cyan
...
if ($process.ExitCode -eq 0) {
  Write-Host "Toontown smoke test passed." -ForegroundColor Green
  exit 0
}

Write-Host ("Toontown smoke test failed with exit code {0}." -f $process.ExitCode) -ForegroundColor Red
